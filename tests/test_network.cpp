...
TEST(NetworkCF, DivisionByZeroDenominatorThrows) {
    Polynomial N(vector<long long>{1});
    Polynomial D; // zero
...
TEST(NetworkCF, Monomial_Z_is_s_only) {
    // N(s) = s, D(s) = 1 → Z=[s], Y=[]
    Polynomial N(vector<long long>{0, 1});
    Polynomial D(vector<long long>{1});
    vector<string> Z, Y;
    cfToZY(N, D, Z, Y);
    ASSERT_EQ(Z.size(), 1u);
    EXPECT_EQ(Z[0], "s");
...
TEST(NetworkCF, SP1_over_S_maps_Zs_Y1) {
    // (s+1)/s → Z=[s], Y=[1]
    Polynomial N(vector<long long>{1, 1});
    Polynomial D(vector<long long>{0, 1});
    vector<string> Z, Y;
    cfToZY(N, D, Z, Y);
    ASSERT_EQ(Z.size(), 1u);
    EXPECT_EQ(Z[0], "s");
...
TEST(NetworkCF, QuadraticOverQuadratic_Zs_YLinear) {
    ...
    Polynomial N(vector<long long>{3, 4, 1});
    Polynomial D(vector<long long>{0, 2, 1});
    vector<string> Z, Y;
    cfToZY(N, D, Z, Y);
    ASSERT_EQ(Z.size(), 1u);
    EXPECT_EQ(Z[0], "s");
...
TEST(NetworkCF, TerminalsNotShorted_nonEmptyZorY) {
    ...
    Polynomial N(vector<long long>{1, 0, 3, 0, 1}); // s^4+3s^2+1
    Polynomial D(vector<long long>{0, 2, 0, 1});    // s^3+2s
    vector<string> Z, Y;
    cfToZY(N, D, Z, Y);
